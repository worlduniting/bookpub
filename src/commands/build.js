 ...
 * 3. **Wipes clean the build output directory** for the selected build type to remove previous build artifacts.
 * 4. Merges the global metadata from `global.meta` with any build-specific metadata from the pipeline configuration.
 * 5. For each stage defined in the pipeline, it merges any global stage settings (from `global.stages`)
 *    with the pipeline-specific overrides. Pipeline settings take precedence.
 * 6. Dynamically imports and executes each stage sequentially.
 *
 ...
 * Also, prior to running the pipeline, the build directory for the selected build type is emptied.
 *
 * If the configuration file or a build pipeline is missing, the process throws an error along with
 * an example configuration snippet to help the user get started.
 *
 * @example
